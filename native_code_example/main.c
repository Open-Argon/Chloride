...
ArgonObject *rational_add(size_t argc, ArgonObject **argv, ArgonError *err,
  ...
  if (api->is_error(err)) return api->ARGON_NULL;
  return api->rational_to_argon((struct rational){a.n*b.d+b.n*a.d, a.d*b.d});
}
...
ArgonObject *yooo(size_t argc, ArgonObject **argv, ArgonError *err,
  ...
  if (argc != 1) {
    return api->throw_argon_error(err, "Runtime Error",
                                  "expected 1 argument, got %" PRIu64, argc);
  }
...
void argon_module_init(ArgonState *vm, ArgonNativeAPI *api,
  ...
  api->register_ArgonObject(reg, "int_add",
                            api->create_argon_native_function("int_add", int_add));
  api->register_ArgonObject(reg, "double_add",
                            api->create_argon_native_function("double_add", double_add));
  api->register_ArgonObject(reg, "rational_add",
  ...
  api->register_ArgonObject(reg, "nice",
                            api->string_to_argon((struct string) {
                              "hello world",
                              11
                            }));
}