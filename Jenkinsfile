    ...
    stages {
        ...
        stage('Setup Conan') {
            steps {
                sh '''
                    apt update
                    apt install -y cmake flex python3 python3-pip python3-venv make gcc-mingw-w64 mingw-w64 ninja-build zip
        ...
        stage('Windows Build') {
            steps {
                sh '''
                  . /tmp/venv/bin/activate
                  rm -rf ~/.conan2 build_mingw CMakeCache.txt CMakeFiles
                  conan install . \
