...
ArgonObject *Argon_new_cursor(size_t argc, ArgonObject **argv,
                                    ArgonError *err, ArgonState *state,
                                    ArgonNativeAPI *api) {
...
ArgonObject *Argon_start_transaction_cursor(size_t argc, ArgonObject **argv,
  ...
  int rc = sqlite3_exec(*(sqlite3**)db.data, "BEGIN TRANSACTION;", NULL, NULL, &err_msg);

  if (rc) {
    ArgonObject*result = api->throw_argon_error(err, "sqlite error", "%s",
                                  err_msg);
...
ArgonObject *Argon_cursor_execute(size_t argc, ArgonObject **argv,
  ...
  int rc = sqlite3_prepare_v2(*(sqlite3**)db.data, query.data, query.length, cursor.data, (const char**)&query.data);
...
void argon_module_init(ArgonState *vm, ArgonNativeAPI *api, ArgonError *err,
  ...
  api->register_ArgonObject(reg, "new_cursor",
                            api->create_argon_native_function(
                                "new_cursor", Argon_new_cursor));
  api->register_ArgonObject(reg, "start_transaction_cursor",
                            api->create_argon_native_function(
                                "start_transaction_cursor", Argon_start_transaction_cursor));
  api->register_ArgonObject(reg, "commit_cursor",
                            api->create_argon_native_function(
                                "commit_cursor", Argon_commit_cursor));
  api->register_ArgonObject(reg, "close_cursor",
                            api->create_argon_native_function(
                                "close_cursor", Argon_close_cursor));
